...
#load table with occurrence counts (calculated by script occRegionFungi)
setwd(wd_table)
...
sps_reg_count$Region[which(sps_reg_count$Region == "Espírito Santo" )] <- "Espírito Santo"
...
#create column with species and region info in the fungi table
sps_reg_list2$sps_reg <- paste0(sps_reg_list2$gbifDarwinCore,"_",
                                sps_reg_list2$FungiRegion)
...
setwd(wd_harmo_cl)
write.csv(sps_reg_list3,"Final_checklist_fungi.csv")
...
perc_confirmed <- ddply(sps_reg_list3,.(FungiRegion),summarise,
                        confirmed=mean(confirmed)*100,
                        n_sps=length(c(FungiRegion)))
...
{
  a <- which(perc_confirmed$FungiRegion == shp2$FungRgn[i])
  if(length(a) > 0)
  {
...
setwd(wd_pts_cont)
write.csv(sps_reg_count3,"Fungi_continent.csv",row.names = F)
...
#check which sps_continent combination in the fungi table have at 
...
perc_modelling <- ddply(sps_reg_list4,.(FungiRegion),summarise,
...
for(i in 1:nrow(shp2))
{
  a <- which(perc_modelling$FungiRegion == shp2$FungRgn[i])
  b <- which(reg_continent$Region == shp2$FungRgn[i])
...
for(i in 1:nrow(shp2))
{
  a <- which(tab_rd_n$Region == shp2$FungRgn[i])
  if(length(a) == 1)
  {
    shp2$Rd[i] <- tab_rd_n$Rd[a]  
  }else{
    shp2$Rd[i] <- ifelse(shp2$FungRgn[i] %in% 
                           sps_reg_list4$FungiRegion,0,NA)
  }
}
...
setwd("C:/Users/ca13kute/Documents/2nd_Chapter/Results/Fungi/Tables")
write.csv(table_res2,"Indices_fungi_region.csv",row.names = F)
